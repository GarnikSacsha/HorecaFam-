...
for (const questionCount of [5, 3]) {
  test(`Employee completes ${questionCount} questions and explicitly restarts the cycle`, async ({
    page,
  }) => {
    ...
    let restarted = false;
    let starts = 0;
    const attempt = { ...baseAttempt, questions: attemptQuestions.slice(0, questionCount) };
    ...
    await page.route("**/api/v1/**", async (route: Route) => {
      ...
      if (
        method === "GET" &&
        pathname === `/me/training/lessons/${lessonId}/interactive-training`
      ) {
        ...
        await route.fulfill({
          json: {
            ...
            assessment_version_id: assessmentVersionId,
            availability: "ready",
            can_start: answerCount < questionCount || restarted,
            reason_codes: [],
            ...
            history: [],
            cycle: {
              id: restarted ? "cycle-2" : "cycle-1",
              number: restarted ? 2 : 1,
              status: answerCount === questionCount && !restarted ? "exhausted" : "available",
              eligible_count: questionCount,
              reserved_count: answerCount,
              answered_count: answerCount,
              remaining_count: restarted ? questionCount : questionCount - answerCount,
              can_restart: answerCount === questionCount && !restarted,
            },
          },
        });
        return;
      ...
      if (
        method === "POST" &&
        pathname === `/me/training/lessons/${lessonId}/interactive-training/cycles/restart`
      ) {
        expectCsrf(request);
        expectIdempotency(request);
        expect(request.postDataJSON()).toEqual({ expected_cycle_id: "cycle-1" });
        restarted = true;
        await route.fulfill({ json: { id: "cycle-2", number: 2 } });
        return;
      }
      if (
        method === "POST" &&
      ...
      if (
        ...
        answerCount += 1;
        const completed = answerCount === questionCount;
        await route.fulfill({
          json: {
            answer: {
    ...
    await expect(
      page.getByText("Кандидата схвалено та опубліковано в Банку питань."),
    ).toBeVisible();
    ...
    for (let questionNumber = 1; questionNumber <= questionCount; questionNumber += 1) {
      await expect(
        page.getByText(`${questionNumber} з ${questionCount}`, { exact: true }),
      ).toBeVisible();
      await expect(
    ...
    await expect(
      page.getByRole("heading", {
        name: `${questionCount} з ${questionCount} правильних відповідей`,
      }),
    ).toBeVisible();
    await expect(page.getByText("Сильне знання · 100%")).toBeVisible();
    await expect(page.getByText("Усі питання пройдено")).toBeVisible();
    await expect(page.getByRole("link", { name: "Продовжити навчання" })).toBeVisible();
    await page.getByRole("button", { name: "Почати новий цикл" }).click();
    await expect(page.getByRole("button", { name: "Почати тренування" })).toBeVisible();
    expect(starts).toBe(1);
    expect(answerCount).toBe(questionCount);
